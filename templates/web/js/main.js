    ...
    $.post(
      ...
      function(data) {
        if (data == 1) {
          ...
          $("#contact-form").after(
            "<h3 style=\"display: none;\" class='form-status'>Ihre Kontaktanfrage wurde erfolgreich versandt.</h3>"
          );
          $(".form-status").slideDown();
        } else {
          $(".g-recaptcha").after(
            "<h3 style=\"display: none;\" class='form-error'>Bitte aktivieren sie das ReCaptcha.</h3>"
          );
...
function initCountdown(selector) {
  $.get("/?async=1&getCountdownData=1", data => {
    ...
    setInterval(function() {
      ...
      container
        .find(".countdown__tile--days")
        .html(days + "<span class='countdown__span'>Tage</span>");
      container
        .find(".countdown__tile--hours")
        .html(hours + "<span class='countdown__span'>Stunden</span>");
      container
        .find(".countdown__tile--minutes")
        .html(minutes + "<span class='countdown__span'>Minuten</span>");
      container
        .find(".countdown__tile--seconds")
        .html(seconds + "<span class='countdown__span'>Sekunden</span>");
    }, 1000);
  });
}
